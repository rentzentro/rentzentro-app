...
export default function LandlordLoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (signInError) {
        console.error('Landlord login error:', signInError);
        setError(signInError.message);
        setLoading(false);
        return;
      }

      // ✅ Success – go to landlord dashboard
      router.push('/landlord');
    } catch (err: any) {
      console.error('Unexpected landlord login error:', err);
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-400">
            Use the email and password you created for your RentZentro landlord account.
          </p>
        </div>
        ...
        <p className="text-[11px] text-slate-500">
          Don&apos;t see your properties yet? Make sure you&apos;re signing in with the
          same email you used when setting up your landlord account.
        </p>
      </div>
    </main>
  );
